  ...
  function loop(el, iteratorNames, evaluateItems, evaluateKey) {
    ...
    evaluateItems()((items) => {
      ...
      let things = items.map((item, index) => {
        ...
        let element = oldThings.find((i2) => i2.key === key)?.element;
        if (element) {
      ...
      let unusedThings = oldThings.filter((i2) => !things.map((i3) => i3.key).includes(i2.key));
      unusedThings.forEach((thing) => thing.remove());
      templateEl._x_old_things = things;
      queueMicrotask(() => {
        templateEl.after(...things.map((i2) => i2.element));
      });
    });
  }
