(() => {
  ...
  function getDirectiveHandler(directive2) {
    ...
    return directiveHandlers[directive2.type] || noop;
  }
...
`, el);
  ...
  function src_default2(el, {expression}) {
    let getValue = module_default.evaluateLater(el, expression);
    track(expression, (setMeta) => {
      ...
      module_default.evaluate(el, `${expression} = value`, {scope: {value}});
    });
  }
  ...
  document.addEventListener("alpine:initializing", () => {
    window.Alpine.directive("history", src_default2);
  });
})();
